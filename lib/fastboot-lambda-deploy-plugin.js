...
module.exports = DeployPlugin.extend({
  ...
  requiredConfig: ['environment', 'lambda-function'],
  ...
  _makeLambdaPackage: function(context) {
    ...
    // Copy lambda-package skeleton to tmp/
    return fs.copy(packageSkeletonPath, 'tmp/lambda-package')
      .then(function() {
        // Unzip pre-cooked node_modules for lambda-packge (for Amazon Linux-native contextify dependency)
        return exec('unzip -o node_modules.zip', { cwd: 'tmp/lambda-package' })
      })
      .then(function() {
        // Delete node_modules zip
        return fs.remove('tmp/lambda-package/node_modules.zip')
      })
      .then(function() {
        // Copy application's fastboot-dist build
        return fs.copy(context.fastbootDistDir, 'tmp/lambda-package/fastboot-dist')
      })
      .then(function() {
        // Zip up entire package
